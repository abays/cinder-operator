...
func (in *CinderSpec) DeepCopyInto(out *CinderSpec) {
	*out = *in
	out.PasswordSelectors = in.PasswordSelectors
	if in.CinderAPINodeSelector != nil {
		in, out := &in.CinderAPINodeSelector, &out.CinderAPINodeSelector
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.CinderSchedulerNodeSelector != nil {
		in, out := &in.CinderSchedulerNodeSelector, &out.CinderSchedulerNodeSelector
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.CinderBackupNodeSelector != nil {
		in, out := &in.CinderBackupNodeSelector, &out.CinderBackupNodeSelector
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
